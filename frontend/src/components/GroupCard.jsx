...
const GroupCard = ({ group, onUpdate }) => {
    ...
    const handleJoin = async () => {
        ...
        try {
            ...
            if (error.response?.data?.detail === 'Already a member of this group') {
                setIsMember(true);
                setMembershipError('Вы уже участник этой группы');
            } else {
                setMembershipError(error.response?.data?.detail || 'Ошибка присоединения');
            }
        } finally {
            setIsLoading(false);
        }
    ...
    const handleLeave = async () => {
        try {
            ...
            if (error.response?.data?.detail === 'Not a member of this group') {
                setIsMember(false);
                setMembershipError('Вы уже не участник этой группы');
            } else if (error.response?.data?.detail === 'Organizer cannot leave the group') {
                setMembershipError('Организатор не может покинуть группу');
            } else {
                setMembershipError(error.response?.data?.detail || 'Ошибка выхода из группы');
            }
        } finally {
            setIsLoading(false);
        }
